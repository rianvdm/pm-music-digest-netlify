    ...
    if (nowPlaying[0].hasOwnProperty('@attr') && nowPlaying[0]['@attr'].hasOwnProperty('nowplaying') && nowPlaying[0]['@attr'].nowplaying === 'true') {
      ...
      html = `
        <div class="track_none">
          <p style="text-align: center;">Right now I am listening to <strong><a href="/search-song?song=${nowPlaying[0].name}%20${nowPlaying[0].artist['#text']}">${nowPlaying[0].name}</strong></a> by <strong><a href="/search?artist=${nowPlaying[0].artist['#text']}">${nowPlaying[0].artist['#text']}</strong></a> from the album <strong><a href="/search-album?album=${nowPlaying[0].album['#text']}%20${nowPlaying[0].artist['#text']}">${nowPlaying[0].album['#text']}</a></strong>.
          ${additionalInfo}</p>
        </div>
      `;
    } else {
