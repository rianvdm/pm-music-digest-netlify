...
async function performSearch(artistName) {
  ...
  if (jsonResponse.data && jsonResponse.data.items && jsonResponse.data.items.length > 0) {
    ...
    // Start OpenAI call
      const prompt = `Write a summary to help someone decide if they might like the artist ${artist.name}. Include information about the artist’s history, members, genres, and styles.`;
