...
fetchLovedTracksJSON('/.netlify/functions/getLovedTracks?limit=5')
  .then(async data => {
    ...
    const html = trackData.map(({ track, lastfmGenres, lastfmGenres2, similarArtist, spotifyUrl, spotifyImgUrl, spotifyYear }) => {
      ...
      return `
        <div class="track_ul">
          <a href="${spotifyUrl}"><img src="${spotifyImgUrl}"></a>
          <div class="no-wrap-text">
            <strong><a href="${spotifyUrl}">${track.name}</a></strong> by <strong><a href="/search?artist=${track.artist.name}">${track.artist.name}</a></strong> (liked on ${formattedDate}).
